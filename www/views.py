...
class Airflow(AirflowViewMixin, BaseView):
    ...
    @expose('/fileimport', methods=['POST'])
    @login_required
    @wwwutils.action_logging
    def fileimport(self):
        ...
        if request.method == "POST":
            ...
            if '.csv' in f.filename:
                upload_folder = './data_access/customer_csv/'
                f.save(upload_folder + secure_filename(f.filename))
                text = f.filename + ' upload Succeed'

            elif '.pdf' in f.filename:
                upload_folder = './data_access/company_pdf/'
                f.save(upload_folder + secure_filename(f.filename))
                text = f.filename + ' upload Succeed'

            elif '.xls' in f.filename:
                upload_folder = './data_access/company_excel/'
                f.save(upload_folder + secure_filename(f.filename))
                text = f.filename + ' upload Succeed'
            ...
            return self.render('airflow/upload.html', text=text)


    @expose('/fileexport/<filename>', methods=['GET'])
    @login_required
    @wwwutils.action_logging
    def fileexport(self, filename):
        print(filename)
        directory = '/dbdv2/data_access/export/'
...
class UploadView(wwwutils.LoginMixin, AirflowViewMixin, BaseView):
    @expose('/')
    def upload_file(self):
        return self.render('airflow/upload.html')

class DownloadView(wwwutils.LoginMixin, AirflowViewMixin, BaseView):
    @expose('/')
    def upload_file(self):
        req_path = './data_access/export/'

        if not os.path.exists(req_path):
            return abort(404)


        files = self.listdir_nohidden(req_path)

        return self.render('airflow/download.html', files = files)
